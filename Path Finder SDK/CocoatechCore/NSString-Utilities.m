...
{
	NSInteger len = MIN((NSUInteger)255, strlen([self UTF8String]));
...
{
	NSInteger len = MIN((NSUInteger)maxLength, strlen([self UTF8String]));
...
+ (NSString*)unique;
{	
	static NSUInteger counter=0;  // protected for thread safety
	NSUInteger intValue;
	...
	return [NSString stringWithFormat:@"u:%lu", intValue];
}
...
{	
	if ([self length])
	{
		...
		const char* fsr = [self fileSystemRepresentation];
		if (fsr)
			result = realpath(fsr, resolved);
...
- (NSData*)nullTerminatedDataUsingEncoding:(NSStringEncoding)theEncoding;
{
	...
	NSData* theData = [self dataUsingEncoding:theEncoding];
	if (theData)
		[result appendData:theData];
