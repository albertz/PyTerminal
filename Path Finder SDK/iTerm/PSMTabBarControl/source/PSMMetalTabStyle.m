...
- (void)drawWindowBackgroundInRect:(NSRect)aRect inView:(NSView*)controlView;
{
	// added for view transition animation.  Otherwise background was black and looked ugly
	[[NSColor grayColor] set];
	[NSBezierPath fillRect:aRect];

	if (([[controlView window] styleMask] & NSHUDWindowMask) == NSHUDWindowMask)
	{
...
- (void)drawTabCell:(PSMTabBarCell *)cell
{
    ...
    if ([cell state] == NSOnState) {
		...
		if (orientation == PSMTabBarHorizontalOrientation) {
			...
			[self drawWindowBackgroundInRect:aRect inView:[cell controlView]];
			aRect.size.width++;
			aRect.size.height += 0.5;
...
- (void)drawTabBar:(PSMTabBarControl *)bar inRect:(NSRect)rect drawLineAtBottom:(BOOL)drawLineAtBottom;
{
	...
	if (([[bar window] styleMask] & NSHUDWindowMask) == NSHUDWindowMask)
	{
		[[NSColor grayColor] set];
		NSRectFillUsingOperation(rect, NSCompositeSourceAtop);
	}	
