...
- (void)initAddedProperties
{
    ...
    if (_addTabButton)
	{
        ...
        [_addTabButton setAlternateTitle:@""];
        ...
        [[_addTabButton cell] setBezeled:NO];
	//	[[_addTabButton cell] setShowsStateBy:NSContentsCellMask];
	//	[[_addTabButton cell] setHighlightsBy:NSPushInCellMask];
...
{
 	[style drawTabBar:self inRect:[self bounds] drawLineAtBottom:[self tabLocation] != PSMTab_BottomTab];
}
    ...
    for (i = 0; i < cellCount; i++) {
        ...
        if (i < numberOfVisibleCells) {
            ...
            if ([[cell representedObject] isEqualTo:[tabView selectedTabViewItem]]) {
                ...
                if (i > 0) {
                    if ([(PSMTabBarCell*)[_cells objectAtIndex:i-1] state] == NSOnState){
...
- (void)_setupOverflowMenu:(NSMenu *)overflowMenu
{
	...
	if (overflowMenu) {
		...
		for (i = 0; equal && i < [overflowMenu numberOfItems]; i++) {
			NSMenuItem* currentItem = [[_overflowPopUpButton menu] itemAtIndex:i], *newItem = [overflowMenu itemAtIndex:i];
...
- (void)tabViewDidChangeNumberOfTabViewItems:(NSTabView *)aTabView
{
	...
	NSArray* cellsCopy = [[_cells copy] autorelease];
    NSEnumerator *e = [cellsCopy objectEnumerator];
    PSMTabBarCell *cell;
    while ( (cell = [e nextObject]) ) {
